...
if(!['status','resume-broadcast','cancel-unsigned','replace-fees','reconcile-nonce','retry-escrow','gas-allowance'].includes(mode)||typeof options['--id']!=='string')throw Error('Use --help for valid arguments.');
...
try{
  ...
  if(mode==='resume-broadcast'){
    if(!options['--owner']||!/^0x[0-9a-fA-F]{64}$/.test(options['--hash']??''))throw Error('Owner and original transaction hash required.');
    result=await (await import('../lib/otc/recovery-runtime.ts')).resumePausedTransaction(id,options['--owner'],options['--hash']);
  }
  ...
  console.log(JSON.stringify({id:result.id,status:result.status,wallet:result.wallet,hash:result.hash,signingStarted:result.signingStartedAt!==undefined,previousHashes:result.previousSigned?.map(a=>a.hash),nonceConflict:result.nonceConflict,broadcastPaused:result.broadcastPausedAt!==undefined,nonceSearch:result.nonceSearch,note:result.note},null,2));
}catch{
  console.error('Recovery did not complete. Check the request status, owner, balance, gas budget and mined receipt. An unresolved signature may still execute; do not submit a duplicate payment.');process.exitCode=1;
}
