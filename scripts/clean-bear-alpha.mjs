...
const {data,info} = await sharp('public/brand/arctos-bear-logo.png').removeAlpha().raw().toBuffer({resolveWithObject:true});
...
await sharp(out,{raw:{width:w,height:h,channels:4}}).png().toFile('public/brand/arctos-bear-transparent.png');
await copyFile('public/brand/arctos-bear-transparent.png','../Arctos Bear Transparent.png');
console.log('Saved clean RGBA bear. Retained component areas:',sizes.filter(x=>x>=50));
