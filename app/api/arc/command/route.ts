  ...
  try{
    ...
    for(let step=0;step<5;step++){
      ...
      if(tx){
        ...
        if(tx.leg!=="allowance")return json({ok:true,message:"Arc transaction confirmed.",hash:tx.hash});
        continue;
      }
      if(Date.now()-auth.createdAt>10*60_000)throw new Error("Request expired. Send a new command after checking wallet history.");
      let prepared:Awaited<ReturnType<typeof prepareCall>> & {leg:"send"|"swap"|"allowance";swapOutput?:{token:string;minimum:string}};
      if(command.kind==="buy"||command.kind==="sell"||command.kind==="swap_token_for_token"){
        if(command.kind==="buy"&&command.unit!=="usd")throw new Error("Specify the USDC amount to spend, for example buy $10 of TOKEN.");
        if(command.kind==="sell"&&!["token","percent"].includes(command.unit))throw new Error("Specify the token amount or percentage to sell.");
        ...
        prepared=await previewArcTrade(wallet,{tokenIn:command.kind==="buy"?"native":target,tokenOut:command.kind==="buy"?target:command.kind==="swap_token_for_token"?token(command.toToken):"native",amount,slippageBps:command.slippageBps});
