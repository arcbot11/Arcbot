...
function failure(e: unknown) {
  ...
  return json({ error: "Launch preparation could not be completed. No transaction was submitted." }, 503);
}
function backend() {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return draft ? json(draft) : unavailable();
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const preview = await prepareLaunch({ identity, input: draft.input, tokenSalt: draft.tokenSalt, config, rpc: createArcRpc(config),
