...
export async function POST(request: NextRequest) {
  ...
  try {
    const { action } = await boundedJson<{ action?: string }>(request, 512);
    ...
    if (action === "start") {
      ...
      await client.mutation(api.telegramWebAuth.start, { secret, tokenHash: hash(token), browserHash: hash(verifier), code, browserFamily: family, sourceHash: loginSourceHash(request, secret), ...(previous ? { previousSessionHash: hashAuth(previous.sessionId) } : {}) });
    ...
    const result = await client.mutation(api.telegramWebAuth.exchange, { secret, tokenHash: hash(token), browserHash: hash(verifier), sessionIdHash: hash(sessionId), browserFamily: family });
