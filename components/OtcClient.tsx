...
type Quote={id:string;amount:string;premiumBps:number;sellerWei:string;feeWei:string;totalWei:string;baseGasWei:string;expiresAt:number;status:string};
  ...
  return <div className="otc-workspace">
    ...
    <div className="otc-single">
      ...
      {formOpen&&<section className="otc-form-panel" id="otc-order-form">
        ...
        <h2 id="otc-form-title">{tab==="buy"?"Base ETH → Arc USDC":"List your Arc USDC"}</h2>
        ...
        {quote&&<div className="otc-quote"><div className="otc-panel-title"><h3>Your quote</h3><span>{Math.max(0,Math.ceil((quote.expiresAt-now)/1000))}s left</span></div><dl><dt>You receive</dt><dd>{units(quote.amount)} Arc USDC</dd><dt>Premium</dt><dd>{pct(quote.premiumBps)}</dd><dt>Seller receives</dt><dd>{units(quote.sellerWei,18)} ETH</dd><dt>Service fee · 1%</dt><dd>{units(quote.feeWei,18)} ETH</dd><dt>Base gas allowance</dt><dd>{units(quote.baseGasWei,18)} ETH</dd><dt>Reserved for this purchase</dt><dd>{units((BigInt(quote.totalWei)+BigInt(quote.baseGasWei)).toString(),18)} ETH</dd></dl><p className="otc-fine">The fee applies after the premium. Unused gas stays in your wallet. Base fees can vary. Arc payout follows verified Base payment.</p><button className="arc-button" disabled={busy||now>=quote.expiresAt||quote.status!=="quoted"} onClick={async()=>{const result=await run({action:"accept",orderId:quote.id});if(result){setQuote(null);setNotice(`Order recorded: ${result.status.replaceAll("_"," ")}. Follow it on your wallet page.`);}}}>Confirm purchase</button></div>}
