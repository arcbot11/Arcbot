...
type Listing={id:string;seller:string;available:string;premiumBps:number;settling?:boolean};
      ...
      {<section className="otc-book"><div className="otc-panel-title"><h2>Listings</h2><button className="arc-button" onClick={()=>openForm("sell")}>Sell USDC</button></div>
        <div className="otc-market-cards">{market?.listings.map(l=>{
          ...
          return <article className="otc-market-card" key={l.id}>
            ...
            <button className="arc-button" disabled={marketError||busy||!!processingPurchase||l.settling||BigInt(l.available)<10000000n} onClick={()=>openForm("buy",l.id)}>{l.settling?"Purchase processing":own?"Your listing":"Buy USDC ↗"}</button>
