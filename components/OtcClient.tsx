...
export function OtcClient(){
  ...
  useEffect(()=>{
    ...
    const check=async()=>{
      ...
      }catch{/* Keep processing until the saved order can be verified. */}
      finally{pending=false;}
    };
  ...
  return <div className="otc-workspace">
    ...
    <div className="otc-single">
      ...
      {formOpen&&<section className="otc-form-panel" id="otc-order-form">
        ...
        {!session?.authenticated&&<p className="otc-notice"><WalletSignInButton className="arc-text-link" destination="/otc">Connect your account</WalletSignInButton> to create a listing or trade. Base actions are website only.</p>}
