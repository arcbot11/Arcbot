...
export function LaunchReview({ input, wallet, walletLabel, preview, expired = false }: {
  ...
  return <section className={styles.panel} aria-label="Launch review">
    ...
    <div className={styles.token}>
      <ExternalTokenImage src={`https://ipfs.io/ipfs/${input.imageURI.slice(7)}`} name={input.name} />
    ...
    <dl className={styles.facts}>
      ...
      <div><dt>Initial creator buy</dt><dd>{displayUsdc(input.devBuyUSDC)} USDC</dd></div>
    ...
    {preview ? <div className={styles.simulation}>
      ...
      <ol>{preview.steps.map(step => <li key={step.kind}>
        {step.kind === "rewards" ? "Configure holder rewards" : step.kind === "approval" ? "Approve creator-buy USDC" : "Create token and pool"}
      </li>)}</ol>
