...
export function HoldingCard({token,onError,onTrade,busy}:{token:ArcTokenBalance;onError:(message:string)=>void;onTrade?:TradeAction;busy:boolean}){
  ...
  return <article className="arc-holding-card">
    ...
    <div className="arc-holding-amount"><span>Balance</span><strong>{balance}</strong><small>{token.symbol} · <span title={token.pricedAt?`Explorer price as of ${token.pricedAt}`:undefined}>{formatTokenUsd(token.usdValue)}</span></small></div>
