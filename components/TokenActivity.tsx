...
export function TokenActivity({ tokenAddress, poolAddress, symbol, currentMarketCapUsd, currentMarketCapUpdatedAt, volume24hUsd, graduated, artwork, summary, details }: {
  ...
  return <TokenMarketSnapshotContext.Provider value={{ marketCapUsd: liveMarketCapUsd, graduated: liveGraduated }}><section className="token-market-panel">
    ...
    <div className="activity-table-wrap"><table className="activity-table">{tab === "trades" ? <><thead><tr><th>Account</th><th>Type</th><th>Amount</th><th>Value</th><th>MCap</th><th>Time</th><th>Txn</th></tr></thead><tbody>{trades.map((trade) => <tr key={`${trade.source}:${trade.id}`}><td><span>{shortAddress(trade.walletAddress)}</span></td><td><span className={`activity-kind ${trade.kind}`}>{capitalize(trade.kind)}</span></td><td>{formatAmount(trade.tokenAmount)} ${symbol}</td><td>{trade.usdAmount === undefined ? "—" : formatMoney(trade.usdAmount)}</td><td>{trade.marketCapUsd === undefined ? "—" : formatMoney(trade.marketCapUsd)}</td><td>{relativeTime(trade.timestamp)}</td><td><span title="Historical transaction">{shortHash(trade.transactionHash)}</span></td></tr>)}</tbody></> : <><thead><tr><th>Rank</th><th>Account</th><th>Holding</th><th>Value</th><th>Share</th></tr></thead><tbody>{holders.map((holder, index) => <tr key={holder.address}><td>{index + 1}</td><td><span>{shortAddress(holder.address)}</span>{holder.tag ? <span className={`holder-tag ${holderTagClass(holder.tag)}`}>{holder.tag}</span> : null}</td><td>{formatAmount(holder.amount)} ${symbol}</td><td>{liveMarketCapUsd === undefined || holder.percentage === undefined ? "—" : formatMoney(liveMarketCapUsd * holder.percentage / 100)}</td><td>{holder.percentage === undefined ? "—" : `${holder.percentage.toFixed(2)}%`}</td></tr>)}</tbody></>}</table></div>
