...
export function WalletControlsPreview({ loading = false }: { loading?: boolean }) {
  return <div className="wallet-dashboard" aria-busy={loading}>
    ...
    <div className="wallet-preview-muted">{["Your OTC listings", "OTC orders", "Transactions"].map(title => <section className="otc-history" key={title}><h2>{title}</h2><p className="otc-fine">Connect X to view records.</p></section>)}</div>
  </div>;
}
