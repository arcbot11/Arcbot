...
export function WalletControlsPreview({ loading = false }: { loading?: boolean }) {
  return <div className="wallet-dashboard" aria-busy={loading}>
    ...
    <div className="otc-panel-title"><div><h2 id="move-funds" style={{scrollMarginTop:100}}>Move funds</h2>{!loading&&<small><WalletSignInButton className="arc-text-link" destination="/wallet#move-funds">Connect wallet</WalletSignInButton></small>}</div></div>
