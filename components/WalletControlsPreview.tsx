...
export function WalletControlsPreview({ loading = false }: { loading?: boolean }) {
  return <div className="wallet-dashboard" aria-busy={loading}>
    <div className="wallet-connection-slot">{loading ? <span className="otc-fine">Checking X connection…</span> : <a className="arc-text-link" href="/api/auth/x/start?returnTo=/wallet">Connect X to use your wallet ↗</a>}</div>
    <div className="otc-wallet-balances wallet-preview-muted"><article><p className="arc-kicker">ARC / USDC</p><h2>— <small>USDC</small></h2><span>Available</span><dl><dt>Reserved</dt><dd>—</dd><dt>Total balance</dt><dd>—</dd></dl></article></div>
