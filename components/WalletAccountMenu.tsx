...
export function WalletAccountMenu(){
 ...
 if(!session?.authenticated)return <a className="arc-nav-cta" href="/wallet">Open wallet ↗</a>;
 return <details className="wallet-account-menu"><summary className="arc-nav-cta">Wallet ▾</summary><div className="wallet-account-dropdown"><strong>@{session.username}</strong><a href="/wallet">Your funds</a><a href="/api/auth/x/start?returnTo=/wallet">Refresh X sign-in</a><button type="button" disabled={busy} onClick={()=>void signOut()}>{busy?"Signing out…":"Sign out"}</button>{error&&<p role="alert">{error}</p>}</div></details>;
}
