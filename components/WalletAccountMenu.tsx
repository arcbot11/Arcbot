...
export function WalletAccountMenu(){
 ...
 return <details className="wallet-account-menu"><summary className="arc-nav-cta">Wallet ▾</summary><div className="wallet-account-dropdown"><strong>{session.provider === "telegram" ? "TG linked wallet" : "@"+session.username}</strong><Link href="/wallet">Your wallet</Link><WalletSignInButton align="right">Refresh sign-in / switch account</WalletSignInButton><button type="button" disabled={busy} onClick={()=>void signOut()}>{busy?"Signing out…":"Sign out"}</button>{error&&<p role="alert">{error}</p>}</div></details>;
}
