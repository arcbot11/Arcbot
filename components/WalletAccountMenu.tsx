...
export function WalletAccountMenu(){
 ...
 return <details className="wallet-account-menu"><summary className="arc-nav-cta">Wallet ▾</summary><div className="wallet-account-dropdown"><strong>@{session.username}</strong><Link href="/wallet">Your wallet</Link><a href="/api/auth/x/start?returnTo=/wallet">Refresh X sign-in</a><button type="button" disabled={busy} onClick={()=>void signOut()}>{busy?"Signing out…":"Sign out"}</button>{error&&<p role="alert">{error}</p>}</div></details>;
}
