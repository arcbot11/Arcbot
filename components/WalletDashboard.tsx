...
import {BaseTokenBalances} from "./BaseTokenBalances";
...
type SendQuote={token?:string;quote:string;amount:string;recipient:string;asset:string;gasWei:string;expiresAt:number};
...
export function WalletDashboard({address}:{address?:string}){
  ...
  const [baseTokenSymbol,setBaseTokenSymbol]=useState("tokens");
  const baseWithdrawal=useRef<HTMLDivElement|null>(null);
  useEffect(()=>{if(tab==="withdraw"&&chain===8453&&asset==="token"){baseWithdrawal.current?.scrollIntoView({block:"center"});baseWithdrawal.current?.focus({preventScroll:true});}},[tab,chain,asset,token]);
  ...
  useEffect(()=>{setSendUnit("tokens");setSendPercent(null);setAmount("");setQuote(null);},[tab,asset,token,session?.walletAddress]);
  ...
  return <div className="wallet-dashboard">
    ...
    {session.walletAddress&&<BaseTokenBalances key={session.walletAddress} address={session.walletAddress} refreshKey={holdingsRefresh} busy={busy} onWithdraw={holding=>{if(busy)return;setTab("withdraw");setChain(8453);setAsset("token");setToken(holding.address);setBaseTokenSymbol(holding.symbol);setSendUnit("tokens");setAmount("");setRecipient("");setQuote(null);}}>
      {tab==="withdraw"&&chain===8453&&asset==="token"&&<div className="base-withdraw-controls" ref={baseWithdrawal} tabIndex={-1}><button type="button" className="otc-inline-button" disabled={busy} onClick={()=>{setTab("buy");setChain(5042);setAsset("native");setQuote(null);}}>Close withdrawal ×</button>{sendControls}<PersistentNotices notices={notices} dismiss={dismiss}/></div>}
    </BaseTokenBalances>}
