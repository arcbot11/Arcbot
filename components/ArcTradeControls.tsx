...
import type {TradeMarket} from "@/lib/arc/markets";
...
export function ArcTradeControls({side,disabled=false,onNotice=()=>{},onBusyChange=()=>{},onCompleted=()=>{},refreshKey=0,initialToken="",children}:{side:"buy"|"sell"|"swap";disabled?:boolean;onNotice?:(message:string)=>void;onBusyChange?:(busy:boolean)=>void;onCompleted?:(id:string)=>void;refreshKey?:number;initialToken?:string;children?:ReactNode}){
  ...
  useEffect(()=>{
    setMarket(null);estimateRouteHint.current=undefined;
    if(side==="swap"||!authenticated||!/^0x[0-9a-fA-F]{40}$/.test(token))return;
    const controller=new AbortController();
    void webPost("/api/wallet/trade",{action:"market",token},{authenticated,walletAddress,csrfToken},controller.signal)
      .then(result=>{if(!controller.signal.aborted)setMarket(result);}).catch(()=>{});
    return()=>controller.abort();
  },[side,token,authenticated,walletAddress,csrfToken]);
  ...
  useEffect(()=>{
    ...
    return startEstimateRefresh<NonNullable<typeof estimate>>({
      ...
      finishInFlightAfterIdle:side!=="swap",
      request:async signal=>{const result=await webPost("/api/wallet/trade",{action:"estimate",...(side!=="swap"?{intent:side}:{}),routeHint:estimateRouteHint.current,tokenIn:side==="buy"?"native":token,tokenOut:side==="buy"?token:side==="sell"?"native":output,amount,amountUnit:side!=="buy"?amountUnit:"tokens",slippageBps:Math.round(Number(slippage)*100)},{authenticated,walletAddress,csrfToken},AbortSignal.any([signal,AbortSignal.timeout(side==="swap"?45000:PAIRED_ESTIMATE_TIMEOUT_MS)]));if(!signal.aborted)estimateRouteHint.current=result.routeHint;return result;},
  ...
  const execute=async()=>{
    ...
    if(side!=="swap"&&(!estimate?.fundingPlan||estimate.expiresAt<=Date.now())){onNotice("Get a fresh estimate before submitting.");return;}
    inFlight.current=true;setActiveFunding(estimate?.funding??null);setCompletion(null);setBusy(true);onBusyChange(true);setProgress(`Preparing ${side}…`);
    try{
      ...
      const initial=await preview(undefined,estimate?.routeHint??estimateRouteHint.current,estimate?.fundingPlan);
      if(side!=="swap"&&initial.fundingPlan!==estimate?.fundingPlan)throw Error("Trade funding changed. Get a new estimate.");
      const outcome=await executeTradeFlow(initial,{preview,status:readTransactionStatus,confirm:q=>webPost("/api/wallet/trade",{action:"confirm",quote:q},session,AbortSignal.timeout(TRADE_REQUEST_TIMEOUT_MS)),wait:()=>new Promise(resolve=>setTimeout(resolve,3000)),active:()=>active.current,progress:message=>setProgress(message.replace(/trade/gi,side))});
  ...
  const funding=busy?activeFunding:estimate?.funding;
  const paired=market?.paired??funding?.paired;
  const receiveSymbol=market?.quote.symbol??funding?.quoteSymbol;
  return <fieldset disabled={disabled||busy} className={`otc-form-panel arc-trade-controls${disabled?" wallet-preview-disabled":""}`}>
    ...
    <label>{side==="buy"?(paired?"USD value to spend":market?"USDC to spend":"Buy amount ($)"):amountUnit==="usd"?(side==="sell"?"USD value to sell":"USD value to swap"):(side==="sell"?"Tokens to sell":"Tokens to swap")}<input inputMode="decimal" value={amount} onChange={e=>{setAmount(e.target.value);}} placeholder="0.00"/></label>
