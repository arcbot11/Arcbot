...
export function ArcTradeControls({side,disabled=false,onNotice=()=>{},onBusyChange=()=>{}}:{side:"buy"|"sell"|"swap";disabled?:boolean;onNotice?:(message:string)=>void;onBusyChange?:(busy:boolean)=>void}){
  ...
  const [estimate,setEstimate]=useState<{minimumOut:string;estimatedTokenDebit?:string;inputTaxAmount?:string;inputTaxBps?:number;inputSymbol?:string|null;outputSymbol?:string|null;outputAddress?:string;expiresAt:number}|null>(null);
  const [estimateStatus,setEstimateStatus]=useState("");
  const authenticated=session?.authenticated,csrfToken=session?.csrfToken,walletAddress=session?.walletAddress;
  const [tokenBalance,setTokenBalance]=useState<{wallet:string;address:string;balance:string;raw:string;maxSellRaw?:string;sellTaxBps?:number;decimals:number;usdValue?:number|null;pricedAt?:string|null}|null>(null);
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(async()=>{
      try{
        const result=await webPost("/api/wallet/trade",{action:"estimate",tokenIn:side==="buy"?"native":token,tokenOut:side==="buy"?token:side==="sell"?"native":output,amount,amountUnit:side==="sell"?amountUnit:"tokens",slippageBps:Math.round(Number(slippage)*100)},{authenticated,walletAddress,csrfToken},AbortSignal.any([controller.signal,AbortSignal.timeout(45000)]));
  ...
  },[side,token,output,amount,amountUnit,slippage,disabled,busy,authenticated,csrfToken,walletAddress]);
  const preview=(fixedAmount?:string)=>webPost("/api/wallet/trade",{action:"preview",tokenIn:side==="buy"?"native":token,tokenOut:side==="buy"?token:side==="sell"?"native":output,amount:fixedAmount??amount,amountUnit:fixedAmount?"tokens":side==="sell"?amountUnit:"tokens",slippageBps:Math.round(Number(slippage)*100)},session,AbortSignal.timeout(125000)) as Promise<Quote>;
  const execute=async()=>{
  ...
  return <fieldset disabled={disabled||busy} className={`otc-form-panel arc-trade-controls${disabled?" wallet-preview-disabled":""}`}>
    ...
    {token&&authenticated&&<p className="otc-fine" aria-live="polite">{selectedBalance?`Balance: ${selectedBalance.balance} · ${formatTokenUsd(selectedBalance.usdValue)}`:balanceFailed?"Balance unavailable.":"Loading balance…"}</p>}
    ...
    {side==="sell"&&<div className="arc-sell-percentages" role="group" aria-label="Sell amount unit">{(["tokens","usd"] as const).map(unit=><button type="button" key={unit} aria-pressed={amountUnit===unit} onClick={()=>{if(unit!==amountUnit){setAmountUnit(unit);setAmount("");}}}>{unit==="tokens"?"Tokens":"$ USD"}</button>)}</div>}
    <label>{side==="buy"?"USDC to spend":side==="sell"?(amountUnit==="usd"?"USD value to sell":"Tokens to sell"):"Tokens to spend"}<input inputMode="decimal" value={amount} onChange={e=>{setAmount(e.target.value);}} placeholder="0.00"/></label>
    ...
    <div className="arc-trade-estimate" aria-live="polite">{estimate?<><p>You will receive at least <strong>{estimate.minimumOut} {estimate.outputSymbol?estimate.outputSymbol:side==="sell"?"USDC":estimate.outputAddress??(side==="buy"?token:output)}</strong>.</p><small>Estimate includes slippage.</small>{side!=="buy"&&estimate.estimatedTokenDebit&&<p>Total token debit: <strong>{estimate.estimatedTokenDebit} {estimate.inputSymbol??"tokens"}</strong>{(estimate.inputTaxBps??0)>0?` (includes ${estimate.inputTaxAmount} token tax)`:""}.</p>}</>:<p>{estimateStatus}</p>}</div>
