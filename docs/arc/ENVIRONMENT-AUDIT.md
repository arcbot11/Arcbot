...
|---|---|---|---|
| `X_API_KEY` | Replace / verify ownership | Configure the Arc Bot X app and authorize @ArcChainBot; verify the numeric user ID. No online account ownership check was performed. | `convex/xReplies.ts`, `scripts/authorize-x.mjs` |
| `X_API_SECRET` | Replace / verify ownership | Configure the Arc Bot X app and authorize @ArcChainBot; verify the numeric user ID. No online account ownership check was performed. | `convex/xReplies.ts`, `scripts/authorize-x.mjs` |
| `X_ACCESS_TOKEN` | Replace / verify ownership | Configure the Arc Bot X app and authorize @ArcChainBot; verify the numeric user ID. No online account ownership check was performed. | `convex/xReplies.ts`, `scripts/authorize-x.mjs` |
| `X_ACCESS_TOKEN_SECRET` | Replace / verify ownership | Configure the Arc Bot X app and authorize @ArcChainBot; verify the numeric user ID. No online account ownership check was performed. | `convex/xReplies.ts`, `scripts/authorize-x.mjs` |
| `X_BOT_USER_ID` | Replace / verify ownership | Configure the Arc Bot X app and authorize @ArcChainBot; verify the numeric user ID. No online account ownership check was performed. | `convex/automatedFeeClaimInfo.ts`, `convex/wallets.ts` |
...
| `OPENROUTER_API_KEY` | Replace recommended | Provider key may be reusable under the same owner, but a dedicated Arc Bot key isolates usage and revocation. Not a chain RPC or proof of Arc token coverage. | `convex/llm.ts`, `scripts/check-arc-apis.mjs` |
...
| `CDP_API_KEY_ID` | Replace | Use dedicated Arc Bot CDP project credentials and its wallet secret. A new key in the old project does not isolate its wallets. | `lib/otc/runtime.ts`, `lib/wallet-signer/service.ts` |
| `CDP_API_KEY_SECRET` | Replace | Use dedicated Arc Bot CDP project credentials and its wallet secret. A new key in the old project does not isolate its wallets. | `lib/otc/runtime.ts`, `lib/wallet-signer/service.ts` |
| `CDP_WALLET_SECRET` | Replace | Use dedicated Arc Bot CDP project credentials and its wallet secret. A new key in the old project does not isolate its wallets. | `lib/otc/runtime.ts`, `lib/wallet-signer/service.ts` |
...
| `NEXT_PUBLIC_CONVEX_URL` | Replace | Use a separate Arc Bot Convex project/deployment. Deployment key belongs in deployment tooling, not client code. | `app/api/auth/x/callback/route.ts`, `app/api/auth/x/session/route.ts` |
...
| `CONVEX_DEPLOY_KEY` | Replace | Use a separate Arc Bot Convex project/deployment. Deployment key belongs in deployment tooling, not client code. | None in scanned application/scripts |
| `CONVEX_DEPLOYMENT` | Replace | Use a separate Arc Bot Convex project/deployment. Deployment key belongs in deployment tooling, not client code. | None in scanned application/scripts |
...
| `COINGECKO_PRO_API_KEY` | Replace recommended | Provider key may be reusable under the same owner, but a dedicated Arc Bot key isolates usage and revocation. Not a chain RPC or proof of Arc token coverage. | `convex/marketData.ts`, `lib/coingecko-client.ts` |
| `OPENROUTER_MANAGEMENT_API_KEY` | Remove from runtime | Operator resource/usage diagnostics only. Keep separately if needed; use Arc Bot-scoped access where supported. Admin keys are not transaction RPC endpoints. | `scripts/check-project-resources.mjs` |
| `ALCHEMY_ADMIN_API_ACCESS_KEY` | Remove from runtime | Operator resource/usage diagnostics only. Keep separately if needed; use Arc Bot-scoped access where supported. Admin keys are not transaction RPC endpoints. | `scripts/check-arc-apis.mjs`, `scripts/check-project-resources.mjs` |
| `VERCEL_ACCESS_TOKEN` | Remove from runtime | Operator resource/usage diagnostics only. Keep separately if needed; use Arc Bot-scoped access where supported. Admin keys are not transaction RPC endpoints. | `scripts/check-arc-apis.mjs`, `scripts/check-project-resources.mjs` |
| `VERCEL_TEAM_ID` | Remove from runtime | Operator resource/usage diagnostics only. Keep separately if needed; use Arc Bot-scoped access where supported. Admin keys are not transaction RPC endpoints. | `scripts/check-project-resources.mjs` |
...
| `TELEGRAM_BOT_TOKEN` | Replace | Token for the dedicated Arc Bot Telegram bot. Do not restore the inherited token. | `convex/telegram.ts`, `scripts/check-arc-apis.mjs` |
| `TELEGRAM_WEBHOOK_SECRET` | Replace | New random secret shared by the Arc Bot webhook and backend. | `app/api/telegram/webhook/route.ts`, `convex/telegram.ts` |
...
| `X_BOT_USERNAME` | Set `ArcChainBot` in the backend; default currently matches. Configure/verify `X_BOT_USER_ID` separately. |
| `NEXT_PUBLIC_ARCBOT_X_URL`, `NEXT_PUBLIC_ARCBOT_TELEGRAM_URL` | Public links to Arc Bot accounts only. |
...
| `OTC_WORKER_URL` | Arc Bot website worker endpoint. Needed by transfer runtime, including regular sends, despite the OTC name. |
| `OTC_BASE_PAYMENT_ROUTER`, `OTC_BASE_ROUTER_CODE_HASH`, `OTC_FEE_WALLET` | Actual Base payment contract, verified deployed bytecode hash, and dedicated Arc Bot service-fee recipient. OTC-only; ordinary transfers do not need these three. |
| `WALLET_SIGNER_URL` | Arc Bot signer endpoint if explicitly configured; never point to the inherited deployment. Legacy signer use still needs Arc migration. |
...
2. Configure Arc Bot website origin, X OAuth login, fresh service secrets, and dedicated posting/Telegram identities.
