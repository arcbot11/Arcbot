...
**Not applied to a remote database.** The local environment still points to the previous project's Convex deployment. Configure and verify a separate Arctos Bot backend before deploying or running this cleanup. Do not run it against the original project's database.
