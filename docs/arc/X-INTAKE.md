...
Direct tags of @ArctosBot bypass the conversation-depth cutoff and the intent restrictions for nested replies. Untagged/inherited thread replies still use the passive-conversation rules. Queued interactions with the old nested-reply flag also honor a direct tag.
...
X_BOT_USERNAME can override the ArctosBot default. Disabled token-creation workflows remain disabled. No live X calls or backend deployment were performed for this change.
