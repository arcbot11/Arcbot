...
- Public links use `https://www.argosbot.io`, `https://x.com/TheArgosBot`, and the new Telegram account supplied by the owner: `https://t.me/The_ArgosBot`. Telegram consent redirects use that same account. This replaces the account checked in the earlier audit; the new bot token and webhook must target the new account.
