...
Built locally, disabled by default. No real wallet key was exported and no deployment, account enrollment, credential change, or bot message was performed. The user requested fresh social verification **without a passkey**; no passkey enrollment or assertion is included.
...
- Only a pre-audited customer registry entry is eligible. The server derives the target from immutable provider IDs and canonical database bindings. Clients cannot submit an address, CDP name, project, or another owner's record as an export selector. Duplicate ownership, stale bindings and protected accounts fail closed. No wallet is created during export.
