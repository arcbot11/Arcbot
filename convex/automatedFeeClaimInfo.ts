...
export const EMPTY_CLAIM_MESSAGES = {
  v2: "Your tokens are Argos Bot V2 tokens. Creator-fee claims and payouts are automated: 95% goes to the creator, while 5% buys back and burns $ARCBOT. You don't need to claim manually.",
  ...
  mixed: "Your Argos Bot V2 launches use automated creator-fee claims and payouts. There are no legacy ETH fees available to claim manually right now. Claim non-ETH-paired legacy tokens individually.",
  pausedV2: "Your Argos Bot V2 tokens use automated creator-fee claims and payouts, but processing is currently paused for one or more tokens. No manual claim was made.",
};
...
export const emptyLegacyClaimMessage = internalQuery({
  ...
  handler: async (ctx, { walletId, tokenAddress }): Promise<Guidance | null> => {
    ...
    for (const program of programs) {
      ...
      return { kind: "v2", message: `ℹ️ ${label} is an Argos Bot V2 token. Creator-fee claims and payouts are automated: 95% goes to the creator, while 5% buys back and burns $ARCBOT.` };
    }
