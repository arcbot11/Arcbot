...
function commandSummary(command: WalletCommand) {
  ...
  if (command.kind === "buy_top_five")
    return `${command.burn ? "Bought and burned" : "Bought"} $${command.amount} each of the top 5 Argos Bot tokens.`;
  if (command.kind === "swap_token_for_token")
...
export const prepareTopFiveWorkflow = internalMutation({
  ...
  handler: async (ctx, args): Promise<TopFiveTarget[]> => {
    ...
    if (args.targets.length !== 5) throw new Error("five eligible Argos Bot tokens were not available");
    const seen = new Set<string>();
...
export function safeFailure(
  ...
  if (
    ...
    return "Failed: You don't have enough of this token's paired asset yet. Buy the paired asset, then reply with the Argos Bot purchase again.";
  ...
  if (
    ...
    return operationKind === "upgrade_fees"
      ? "Required: You don't have the rights to upgrade creator fees for that Argos Bot launch."
      : "Required: You don't have the rights to reassign fees for that Argos Bot launch.";
...
export const executeCommand = internalAction({
  ...
  handler: async (ctx, args): Promise<CommandResult> => {
    ...
    if (command.kind === "create_wallet" || command.kind === "show_wallet") {
      ...
      return {
        ok: true,
        message: `Your Argos Bot wallet is ready.
...
Burn TXN: ${transactionUrl(item.burnTransactionHash)}`] : []),
          ...
          const message = `Confirmed: ${command.burn ? "Bought and burned" : "Bought"} $${command.amount} each of the top 5 Argos Bot tokens.
