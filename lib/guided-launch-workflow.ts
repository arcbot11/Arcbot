...
function explanation(phase: GuidedLaunchPhase) {
  const explanations: Record<GuidedLaunchPhase, string> = {
    ...
    dev_buy: "A developer buy purchases tokens for your Argos Bot wallet in the launch transaction. It is optional and uses your wallet funds.",
    fees: "By default, 95% of creator fees go to your Argos Bot wallet and 5% buys back and burns $ARCBOT. Instead assign your share to another wallet or holders, or use a percentage of it to buy back and burn your launched token.",
...
function summary(state: GuidedLaunchState) {
  ...
  const rows = [
    ...
    `Creator fees: ${d.holderFeeSharing ? "Shared with holders" : d.feeRecipient ? `Assigned to ${d.feeRecipient}` : d.selfBurnBps !== undefined ? `${d.selfBurnBps / 100}% of your share buys back and burns $${d.symbol}` : "Your Argos Bot wallet"}`,
    "",
