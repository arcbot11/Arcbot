...
export function explicitArcSwap(text: string) {
  const match = text.match(tokenPattern(/\bswap\s+(\$)?([0-9][0-9,]*(?:\.[0-9]+)?|\.[0-9]+|all)(%)?(?:\s+(USD|USDC))?\s+(?:(?:worth\s+)?of\s+)?(?:my\s+)?\$?(0x[a-fA-F0-9]{40}|[A-Za-z][A-Za-z0-9_]{0,31})\s+(?:for|to|into)\s+\$?(0x[a-fA-F0-9]{40}|[A-Za-z][A-Za-z0-9_]{0,31})\b/i));
