...
import { exactAmount, UINT256_MAX } from "../arc/amounts.ts";
import type { BaseReceipt } from "./rpc.ts";
...
export type SendRecord = {
  ...
  prepared?: PreparedSend; raw?: Hex; hash?: Hex; receipt?: BaseReceipt;
};
...
export const journalJson = (value: unknown) => JSON.stringify(value, (_, item) => typeof item === "bigint" ? { baseBigInt: item.toString() } : item);
export const parseJournalJson = (text: string): WalletJournal => JSON.parse(text, (_, item) =>
  item && typeof item === "object" && Object.keys(item).length === 1 && typeof item.baseBigInt === "string" ? BigInt(item.baseBigInt) : item);
...
const receiptSchema = z.object({ hash, status: z.enum(["success", "reverted"]), blockNumber: uint,
  blockHash: hash, gasUsed: uint, effectiveGasPrice: uint, l1Fee: uint.optional() }).strict();
const recordSchema = z.object({
  ...
  prepared: z.object({ intent: sendIntent, amountUnits: uint, decimals: z.literal(18),
    snapshot: z.object({ number: uint, hash }).strict(), expiresAt: z.number().int().nonnegative().safe(), gasReserveWei: uint,
    l1FeeUpperBoundWei: uint, operatorFeeWei: uint, delivery: z.literal("native"),
    transaction: z.object({ chainId: z.literal(8453), type: z.literal("eip1559"), to: address, data: hex, value: uint,
...
export function validateWalletJournal(journal: WalletJournal, wallet: Address) {
  ...
  for (const record of journal.records) {
    if (record.intent.from.toLowerCase() !== wallet.toLowerCase() || ids.has(record.intent.requestId)
      || sendDigest(record.intent) !== record.digest) throw new Error("Base journal identity mismatch");
    ...
    if (signed !== !!record.raw || signed !== !!record.hash || (signed && !record.prepared)
      || (record.status === "prepared" && !record.prepared)) throw new Error("Base journal state is incomplete");
    if (record.raw && keccak256(record.raw) !== record.hash) throw new Error("Base journal hash mismatch");
    ...
    if (mined !== !!record.receipt || (record.receipt && (record.receipt.hash !== record.hash
      || (record.receipt.status === "success") !== (record.status === "mined")))) throw new Error("Base journal receipt mismatch");
    if (record.prepared) {
      ...
      if (sendDigest(p.intent) !== record.digest || p.intent.requestId !== record.intent.requestId
        ...
        || tx.maxPriorityFeePerGas > tx.maxFeePerGas || p.gasReserveWei !== tx.gas * tx.maxFeePerGas + 2n * (p.l1FeeUpperBoundWei + p.operatorFeeWei)) {
        throw new Error("Base journal proposal differs from its intent");
      }
    }
  }
}
...
export class FileBaseJournal implements BaseJournal {
  ...
  async withWallet<T>(wallet: Address, action: (journal: WalletJournal, save: () => Promise<void>) => Promise<T>): Promise<T> {
    ...
    const file = join(this.directory, `8453-${wallet.toLowerCase()}.json`);
    const lock = `${file}.lock`;
    let handle;
    try { handle = await open(lock, "wx", 0o600); }
    catch (error) {
      if ((error as NodeJS.ErrnoException).code === "EEXIST") throw new Error("Base wallet is locked; reconcile any stopped worker before clearing its lock");
      throw error;
    }
    try {
