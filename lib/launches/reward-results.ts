...
export type RewardResultContext={splitter:string;tracker:string;creator:string;treasury?:string;token:string;quote:string;payout:string;assets:Record<string,{symbol:string;decimals:number}>};
...
export function rewardResultLines(logs:Log[],c:RewardResultContext){
 ...
 for(const e of parseEventLogs({abi:rewardEvents,logs:logs.filter(l=>same(l.address,c.splitter)),strict:true})){
  ...
  if(e.eventName==="Paid"&&(!c.treasury||!same(e.args.to,c.treasury)))add(same(e.args.to,c.creator)?"Paid to creator":"Other fee payouts",e.args.currency,e.args.amount);
 ...
 return lines.length?lines:["No creator allocations, burns, liquidity additions, or holder payouts were recorded by this transaction."];
}
