import { PORTAL8, portal8Abi, encodePortal8Launch } from './portal8';
...
import { LaunchError, LAUNCH_EXECUTION_ENABLED, LAUNCH_MAX_GAS, PORTAL8_MAX_GAS, LAUNCH_TOTAL_GAS_WEI, LAUNCH_AUTHORIZATION_MS } from "./policy";
...
export function assertLaunchPortal(portal: unknown) {
  if (typeof portal !== "string" || ![LAUNCH_PORTAL.toLowerCase(),PORTAL8.toLowerCase()].includes(portal.toLowerCase()))
...
export function launchCall(terms: LaunchStepTerms) {
  ...
  if(p.portal.toLowerCase()===PORTAL8.toLowerCase()){
    if(!p.quote || terms.kind==='rewards')throw Error('Invalid Portal 8 terms.');
    return terms.kind==='launch'?{to:PORTAL8,data:encodePortal8Launch(input,p.creator,p.hookSalt,p.quote),value:0n}
      :{to:quote.address,data:encodeFunctionData({abi:approvalAbi,functionName:'approve',args:[PORTAL8,amount]}),value:0n};
  }
...
export function assertLaunchTransaction(owner:string,wallet:string,unsigned:Hex,terms:LaunchStepTerms) {
  ...
  const maxGas = launchGasLimit(terms);
...
export function launchGasLimit(terms:LaunchStepTerms){
  return terms.kind==='launch' ? (terms.preview.portal.toLowerCase()===PORTAL8.toLowerCase()?PORTAL8_MAX_GAS:LAUNCH_MAX_GAS) : BigInt(ARC_GAS_POLICY.maxGas);
}
...
export function launchNativeSpend(call:{to?:string|null;data?:Hex;value?:bigint}) {
  if(call.to?.toLowerCase()===PORTAL8.toLowerCase()&&call.data){
    const decoded=decodeFunctionData({abi:portal8Abi,data:call.data});
    if(decoded.functionName!=='launch')throw Error('Unexpected Portal 8 call');
    const p=decoded.args![0] as {quoteAsset:string;bundle:Array<{amountQuote:bigint}>};
    return p.quoteAsset.toLowerCase()===LAUNCH_PAIRS.USDC.address?p.bundle.reduce((n,b)=>n+b.amountQuote,0n)*10n**12n:0n;
  }
