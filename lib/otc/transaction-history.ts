...
export function transactionHistory(record:Transaction){
  ...
  if(record.creatorClaim?.reward){result.title=record.creatorClaim.reward.action==="holders"?"Pay holder rewards":"Distribute token fees";details.push({label:"Token",value:record.creatorClaim.token});}
