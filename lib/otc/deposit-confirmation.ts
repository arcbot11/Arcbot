...
export async function otcDepositConfirmed(client: Client, deposit: Transaction, now = Date.now(), expectedStatus: "success" | "reverted" = "success") {
  ...
  if (!receipt || receipt.status !== expectedStatus || receipt.transactionHash.toLowerCase() !== deposit.hash.toLowerCase()) return false;
