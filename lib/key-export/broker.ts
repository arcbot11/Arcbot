...
export async function encryptedCdpExport(input:{address:string;projectId:string;cdpAccountName:string;publicKey:string;exportId:string},fetcher:typeof fetch=fetch){
  ...
  if(lookup.status===401||lookup.status===403)exportFail("UNAVAILABLE");if(lookup.status===404)exportFail("ELIGIBILITY");
  ...
  if(exported.status===401||exported.status===403)exportFail("UNAVAILABLE");
