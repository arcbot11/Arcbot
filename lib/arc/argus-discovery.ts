...
export async function discoverArgusPool(token:Address,rpc:ArcRpc,block:bigint){
 ...
 for(const portal of ARGUS_PORTALS){
  ...
  const record=decodeFunctionResult({abi:portal.words===10?discoveryAbi:oldAbi,functionName:'launches',data:raw});
  if(record[0]===zeroAddress)continue;
  if(portal.words===10&&await read(portal.address,'LAUNCH_STRUCT_WORDS')!==10)throw new Error('Unexpected Argus Portal format.');
