...
export function clearTradeDiscoveryCache(){discoveryCache.clear();v3Candidates.clear();}
...
async function quoteArcTrade(wallet:Address,input:TradeInput){
  ...
  for(const protocol of ["v3","v4"] as const){
    ...
    if(routes.length)groups.push(...(await quoteRoutes(routes,amountIn,input.slippageBps,wallet,rpc,config)).quotes.filter(q=>!q.executionBlocker||q.route.pools.every(p=>p.protocol!=="v4"||p.hooks===zeroAddress||verifiedHookPoolIds.includes(poolId(p)))));
  ...
  if (!native(input.tokenIn) && !native(input.tokenOut)) {
    ...
    if (routes.length) groups.push(...(await quoteRoutes(routes, exactAmount(input.amount, await rpc.decimals(getAddress(input.tokenIn),head.number)), input.slippageBps, wallet, rpc, config)).quotes);
  }
  ...
  return {q,rpc,client,head,verifiedHookPoolIds,inputTaxBps};

}

export async function estimateArcTrade(wallet:Address,input:TradeInput){
  const {q,rpc,client,head,inputTaxBps}=await quoteArcTrade(wallet,input);
...
export async function arcSellAmountForUsdc(wallet:Address,token:Address,value:string){
  ...
  const {q}=await quoteArcTrade(wallet,{tokenIn:token,tokenOut:"native",amount:formatUnits(sample,decimals),slippageBps:100});
...
export async function previewArcTrade(wallet:Address,input:TradeInput,delivery: boolean | Address=false){
  ...
  const {q,rpc,client,head,verifiedHookPoolIds,inputTaxBps}=await quoteArcTrade(wallet,input);
  ...
  return {...prepared,tradeGasBudgetWei,reserveWei:(BigInt(prepared.reserveWei)+usdcInput).toString(),leg,stage,swapOutput:leg==="swap"?{token:q.route.tokenOut,minimum:q.amountOutMinimum.toString(),...(outputRecipient?{recipient:outputRecipient}:{})}:undefined,
