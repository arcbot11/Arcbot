...
export async function socialAuthority(requestId:string){
  ...
  return new ConvexHttpClient(url).action(makeFunctionReference<"action">("wallets:authorizeArcCommand"),{secret,requestId}) as Promise<{owner:string;wallet:string;command:string;createdAt:number}>;
}
