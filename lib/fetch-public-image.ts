...
async function timedLookup(hostname:string){
  let timer:ReturnType<typeof setTimeout>|undefined;
  try{return await Promise.race([lookup(hostname,{all:true,verbatim:true}),new Promise<never>((_,reject)=>{timer=setTimeout(()=>reject(Error("Image DNS timed out")),4000);})]);}finally{if(timer)clearTimeout(timer);}
}
...
async function validateUrl(raw: string) {
  ...
  const directIp = isIP(hostname) ? [hostname] : (await timedLookup(hostname)).map((entry) => entry.address);
...
export async function fetchPublicImage(raw: string) {
