...
const REFERENCE_IMAGE_PHRASE = /\b(?:use|using|with)\s+(?:this\s+(?:image|picture)|the\s+(?:image|picture)\s+below)\b/i;
const NEGATED_REFERENCE_IMAGE_PHRASE = /\b(?:do not|don't|dont|not|never|without)\b[^.!?\n]{0,32}\b(?:use|using|with)\s+(?:this\s+(?:image|picture)|the\s+(?:image|picture)\s+below)\b/i;
