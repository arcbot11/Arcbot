 ...
 */
export function xBrowserReturn(request:NextRequest,siteUrl:string,completed=false,completionProof?:string) {
  ...
  if(!cookie||(!completed&&(!code||code.length>2048||request.cookies.has(cookie)||request.nextUrl.searchParams.has("browserReturn")))||request.nextUrl.searchParams.has("error"))return null;
  const android=/Android/i.test(ua),ios=/iPhone|iPad|iPod/i.test(ua);
  if(!completed&&!android&&!ios)return null;
  const callback=new URL("/api/auth/x/callback",siteUrl);
  if(callback.protocol!=="https:")return null;
  callback.search=new URLSearchParams(completed?{state,resume:"1",proof:completionProof!}:{state,code:code!,browserReturn:"1"}).toString();
  ...
  const hint=/^v[34]_(ff|ch|other)_/.exec(state)?.[1];
