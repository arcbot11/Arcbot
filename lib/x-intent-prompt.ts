...
export function currentXExtractorPrompt(operation: string): string | null {
  const instructions: Record<string, string> = {
    ...
    buy: 'Return kind buy, amount, unit usd, and token. The amount is an explicit dollar or USDC budget for buying the named token, for example "buy $25 worth of TOKEN". Do not turn an unqualified token quantity or an ETH amount into USDC.',
    ...
    sell: 'Return kind sell, amount, unit usd|token|percent and token. A dollar value means USD worth of that token; a plain quantity means token units. The sale returns Arc USDC.',
