...
describe("separated ticker and contract", () => {
  it("accepts the WSBonHOOD burn and retains its identity verification", () => {
    const text = `Another $WSB burn directly on the timeline! Watch the supply vanish. 🔥 👇 @ArctosBot burn 1000000 ${address} Study the architectural alpha. We're here to stay. 🏁 #WallStBrett #RWA #Memecoin #Arc #TokenBurn`;
  ...
  it.each(["buy $14 of", "sell 10", "burn 10"])("accepts separated identifiers for %s", action => {
    const text = `For $WSB: @ArctosBot ${action} ${address}`;
  ...
  it.each(["wallet", "launch", "send", "swap"])("does not let incidental %s bypass identity checks", word => {
    const text = "My " + word + " story for $WSB. @ArctosBot burn 10 " + address;
