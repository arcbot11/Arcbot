...
describe("block-aware token delivery",()=>{
 it('verifies a taxed send using both debit and delivery state',()=>{const logs=[log(sender,recipient,99n),log(sender,token,1n)];expect(verifyTransferDelivery({token,sender,recipient,amount:100n,before:0n,after:99n,logs,blockLogs:logs,taxedSend:{senderBefore:1000n,senderAfter:900n}})).toBe(99n);});
 it('keeps exact delivery mandatory for OTC',()=>{const logs=[log(sender,recipient,99n),log(sender,token,1n)];expect(()=>verifyTransferDelivery({token,sender,recipient,amount:100n,before:0n,after:99n,logs,blockLogs:logs})).toThrow('Exact');});
 it.each(['missing-fee','wrong-sender-balance','zero-delivery','missing-block'])('rejects incomplete taxed-send evidence: %s',mode=>{const logs=mode==='zero-delivery'?[log(sender,token,100n)]:[log(sender,recipient,99n),...(mode==='missing-fee'?[]:[log(sender,token,1n)])];expect(()=>verifyTransferDelivery({token,sender,recipient,amount:100n,before:0n,after:99n,logs,blockLogs:mode==='missing-block'?undefined:logs,taxedSend:{senderBefore:1000n,senderAfter:mode==='wrong-sender-balance'?901n:900n}})).toThrow();});
 it('reconciles taxed delivery despite unrelated later recipient spending',()=>{const logs=[log(sender,recipient,99n),log(sender,token,1n)];expect(verifyTransferDelivery({token,sender,recipient,amount:100n,before:100n,after:49n,logs,blockLogs:[...logs,log(recipient,token,150n)],taxedSend:{senderBefore:1000n,senderAfter:900n}})).toBe(99n);});
