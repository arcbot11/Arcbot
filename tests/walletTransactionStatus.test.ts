...
beforeEach(()=>{vi.clearAllMocks();m.advance.mockImplementation(()=>m.read());m.session.mockResolvedValue({owner:"alice",walletAddress:address});m.read.mockResolvedValue({kind:"transaction",id:"trade:test",owner:"alice",wallet:address,status:"submitted",leg:"swap",raw:"secret raw bytes",unsigned:"unsigned bytes"});});
