...
describe("fixed Argos Bot configuration",()=>{
 ...
 it("pins X identity despite old environment overrides",()=>{vi.stubEnv('X_BOT_USERNAME','OldBot');vi.stubEnv('NEXT_PUBLIC_ARCBOT_X_URL','https://x.com/OldBot');expect(xBotUsername()).toBe('TheArgosBot');expect(brand.xUrl).toBe('https://x.com/TheArgosBot');});
 ...
 it("derives the worker path from site origin, ignoring stale worker URLs",()=>{vi.stubEnv('OTC_WORKER_URL','https://old.invalid/worker');vi.stubEnv('NEXT_PUBLIC_SITE_URL','https://old.invalid');expect(otcWorkerUrl()).toBe('https://www.argosbot.io/api/otc/worker');expect(otcWorkerUrl('https://preview.example/')).toBe('https://preview.example/api/otc/worker');});
