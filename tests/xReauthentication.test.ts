...
vi.mock("convex/browser",()=>({ConvexHttpClient:class{async action(){return true;}async mutation(){return {generation:1};}}}));
...
const request=()=>new NextRequest("https://www.argosbot.io/api/auth/x/start?returnTo=/otc",{headers:{cookie:`${WEB_WALLET_SESSION_COOKIE}=${cookie}; ${BROWSER_COOKIE}=${browserValue(secret)}`}});
...
it("preserves wallet-specific destinations during login",async()=>{
  ...
  const response=await GET(new NextRequest(`https://www.argosbot.io/api/auth/x/start?returnTo=${encodeURIComponent(path)}`,{headers:{cookie:`${BROWSER_COOKIE}=${browserValue(secret)}`}}));
  const state=new URL(response.headers.get("location")!).searchParams.get("state")!;
  expect(readOAuthAttempt(response.cookies.get(oauthCookieName(state)!)?.value,secret)?.returnTo).toBe(path);
});
