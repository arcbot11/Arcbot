...
function launchBlocks(source: string) {
  ...
  const begins = /^(?:launch\b|launch\s+token\b|create\b|deploy\b|new\s+(?:launch|token|one)\b|i\s+(?:want|need)\b|need\b|can\b|argus\b|token\s*:|token\s+(?:launch|request|name)\b|pls\b|make\b|name\b|ticker\b|\$[A-Z]|attached\b|use\s+the\s+image|hey\s+argus|@arcbot|“[^”]+”|'[^']+'|NAME:|TOKEN LAUNCH REQUEST)/i;
  const blocks: string[] = [];
  let current: string[] = [];
...
describe.runIf(process.env.LIVE_AI_TESTS === "true" && Boolean(sourcePath))("attached launch batch through live AI", () => {
  it("classifies and extracts without X, wallet, or launch execution", async () => {
    ...
    for (let offset = 0; offset < posts.length; offset += 4) {
      ...
      const intents = await Promise.all(batch.map((post) => parseXWalletIntent(`@ArcBot ${post}`, /attach(?:ed|ment)|\bartwork\b|\bimage\b|\blogo\b|\bpic\b/i.test(post))));
