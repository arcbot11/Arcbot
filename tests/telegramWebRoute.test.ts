...
const request=(action:string,cookie="",origin=site)=>new NextRequest(`${site}/api/auth/telegram`,{method:"POST",headers:{origin,cookie,"content-type":"application/json"},body:JSON.stringify({action})});
...
it("replaces and revokes the old X session when the TG wallet is approved",async()=>{
  ...
  expect(getFunctionName(m.action.mock.calls[0][0])).toBe("wallets:revokeWebSession");
  expect(m.action.mock.calls[0][1].ownerXUserId).toBe("456");
...
it("does not issue access when replacing the old session cannot be confirmed",async()=>{
  m.mutation.mockResolvedValue({status:"approved",telegramUserId:"123",walletAddress:wallet,authenticatedAt:Math.floor(Date.now()/1000)});
  m.action.mockRejectedValue(Error("unavailable"));
