...
describe("burn inquiries", () => {
  it("admits the actual STARTUP reply including inherited participant mentions", async () => {
    const text = "@StartupRH_ @TheArgosBot how much $STARTUP 0xcc3cc9ce3a657472f3c10749ab8ebd3885b4b19e has been burned?";
    ...
    expect(shouldHandlePassiveChainText("@StartupRH_ @TheArgosBot so many burns lately")).toBe(false);
  });
  ...
  it.each([
    ...
    "Hey, @TheArgosBot could you tell me how much $WSB has been burned, please?",
  ...
  it.each(["WSB", "$WSB", "$COIN", "CA", ca, `$WSB CA: ${ca}`, "$パペット"])("recognizes %s without transaction authority", async token => {
    const text = `@TheArgosBot How much ${token} has been burned?`;
