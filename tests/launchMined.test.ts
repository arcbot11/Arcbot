...
import { PORTAL6, reviewedImplementations } from "../lib/launches/contracts";
...
const p:LaunchPreview={version:1,executionEnabled:false,portal:PORTAL6,creator,tokenSalt:salt,hookSalt:salt,predictedToken:token,predictedHook:hook,predictedSplitter:splitter,
...
function logs(){return [
  {address:PORTAL6,topics:encodeEventTopics({abi:launchEvents,eventName:"TokenCreated",args:{token,creator}}) as [Hex,...Hex[]],data:encodeAbiParameters(parseAbiParameters("string,string,bytes32,string,string,string,string"),[input.name,input.symbol,pool as Hex,input.imageURI,"","",""])},
  {address:PORTAL6,topics:encodeEventTopics({abi:launchEvents,eventName:"PartsDeployed",args:{token}}) as [Hex,...Hex[]],data:encodeAbiParameters(parseAbiParameters("address,address,address"),[locker,hook,splitter])},
  transfer(ARC_USDC,creator,PORTAL6,25_000_000n),transfer(token,hook,creator,1000n*10n**18n),transfer(ARC_USDC,PORTAL6,creator,3_000_000n),
];}
beforeEach(()=>{
  ...
  m.client.getTransaction.mockResolvedValue({from:creator,to:PORTAL6,value:0n,input:encodeLaunch(input,salt,salt,p.quote)});
...
it("supports native USDC logs without double-counting matching ERC-20 logs",async()=>{
  const receipt=await m.client.getTransactionReceipt();receipt.logs.push(transfer(ARC_NATIVE_TRANSFER,creator,PORTAL6,25n*10n**18n),transfer(ARC_NATIVE_TRANSFER,PORTAL6,creator,3n*10n**18n));
...
it.each(["reorg","implementation","allocation","quote","delivery","refund"])("rejects unverified %s",async reason=>{
  ...
  if(reason==="refund")receipt.logs.push(transfer(ARC_USDC,PORTAL6,creator,30_000_000n));
