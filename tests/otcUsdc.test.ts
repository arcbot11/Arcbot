...
import {type Store,type RecordValue,type Wallet,type Order,createListing,createQuote,acceptQuote,locked,lockedBaseUsdc,walletId,usdcPrice,finishOrder} from "../lib/otc/model";
...
async function accepted(){const s=await fixture();const order=await createQuote(s,input,now);await acceptQuote(s,order.id,"buyer",snapshot,now);return s;}
...
describe("Base USDC OTC",()=>{
 ...
 it("requires USDC and ETH separately",async()=>{const s=await fixture();await expect(createQuote(s,{...input,baseUsdcBalance:"11109999"},now)).rejects.toThrow("Base USDC");await expect(createQuote(s,{...input,baseBalanceWei:"199"},now)).rejects.toThrow("Base ETH");});
 ...
 it("rechecks token funds at acceptance",async()=>{const s=await fixture();await createQuote(s,input,now);await expect(acceptQuote(s,input.id,"buyer",{...snapshot,baseUsdcBalance:"0"},now)).rejects.toThrow("Base USDC");expect((await s.get<Order>(input.id))?.status).toBe("quoted");});
 it("prevents concurrent approvals overwriting an accepted purchase",async()=>{const s=await accepted();await createQuote(s,{...input,id:"order:two",baseUsdcBalance:"999999999",baseBalanceWei:"10000"},now);await expect(acceptQuote(s,"order:two","buyer",{...snapshot,baseUsdcBalance:"999999999",baseBalanceWei:"10000"},now)).rejects.toThrow("purchase is pending");});
