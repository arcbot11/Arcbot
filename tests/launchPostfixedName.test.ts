...
it.each([
  '@ArcBot launch token Hermes name ticker Herman @ArcBot',
  'launch token Hermes name ticker Herman',
  'launch token name Hermes ticker Herman @ArcBot',
  'launch token Hermes name: ticker: Herman',
...
it('keeps fee recipients and developer buys unchanged', () => {
  expect(groundedCanonicalCommand('launch token Hermes name ticker Herman assign fees to @ArcBot buy $20')).toMatchObject({ name: 'Hermes', symbol: 'HERMAN', devBuy: { amount: '20', unit: 'usd' } });
});
