...
describe("Arc snapshot index selection", () => {
  ...
  it("retains the highest-cap contract for each ticker in the combined snapshot", () => {
    ...
    for (const selected of ARC_TOKEN_CATALOG.filter(t => t.symbol !== "USDC" && contracts.has(t.address) && !latest.added.some(n => n.address === t.address))) {
  ...
  it("adds verified recent launches once and excludes their rejected duplicates", () => {
    ...
    for (const token of additions.added) {
      if ([...refresh.replaced, ...latest.replaced].some(t => t.previous === token.address)) continue;
  ...
  it("admits only screened, on-chain verified additions and retires replaced ticker addresses", () => {
    for (const token of refresh.added) {
      if (latest.replaced.some(t => t.previous === token.address)) continue;
  ...
  it("screens the latest explorer additions without claiming Argus launch provenance", () => {
    for (const token of latest.added) {
      expect(ARC_TOKEN_CATALOG.find(t => t.address === token.address)).toEqual(token);
      expect(token.argus).toBe(false);
      const evidence = latest.verified.find(t => t.address === token.address)!;
      expect(evidence.volume24h).toBeGreaterThanOrEqual(latest.screen.minVolume24hUsd);
      expect(evidence.liquidityUsd).toBeGreaterThanOrEqual(latest.screen.minLiquidityUsd);
      expect(evidence.trades).toBeGreaterThanOrEqual(latest.screen.minTrades);
      expect(evidence.volume24h / evidence.marketCap).toBeGreaterThanOrEqual(latest.screen.minVolumeToMarketCap);
    }
    for (const token of latest.replaced) expect(canIndexArcToken(token.previous, token.symbol)).toBe(false);
  });
  ...
  it("seeds idempotently, removes rejected index rows, and never enables launch-pair approval", async () => {
    ...
    tables.tokenRegistry = [];
    tables.walletTokenIndex = Array.from({ length: 110 }, (_, i) => ({ _id: `rejected${i}`, normalizedTokenAddress: excluded[0] }));
    await expect(refreshCatalogBatch(ctx, -1)).rejects.toThrow("Invalid catalog offset");
    await expect(refreshCatalogBatch(ctx, 1.5)).rejects.toThrow("Invalid catalog offset");
