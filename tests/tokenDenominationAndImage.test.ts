...
describe("whole image instructions are not ticker values", () => {
  ...
  it.each([
    ["@TheArgosBot launch a token $TOAST TOSTA ticker Use this image as logo Add Your Token Logo.", "TOAST"],
    ["@TheArgosBot launch a token mosfik_eth ticker Use this image as logo", "MOSFIKETH"],
  ...
  it.each([true, false])("rejects a hallucinated AS even with hasImage=%s", async hasImage => {
    ...
    const result = await parseXWalletIntent("@TheArgosBot launch River ticker use this image as logo", hasImage, diagnostics);
