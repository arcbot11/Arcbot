...
it("waits 500ms, expires old quotes and refreshes no more than once a minute", async () => {
  ...
  await vi.advanceTimersByTimeAsync(30_000); expect(options.request).toHaveBeenCalledTimes(2);
  expect(options.estimate).toHaveBeenLastCalledWith({ expiresAt: 90_500, minimumOut: "10" });
  stop();
});
it("stops at two minutes instead of extending the deadline on refresh", async () => {
  ...
  await vi.advanceTimersByTimeAsync(300_000); expect(options.request).toHaveBeenCalledTimes(2);
});
...
it("bounds retries when the provider returns already-expired estimates", async () => {
  ...
  expect(options.request.mock.calls.length).toBeLessThanOrEqual(2);
  expect(options.status).toHaveBeenLastCalledWith("Estimates paused. Change the amount to refresh.");
});
