...
beforeEach(()=>{vi.resetAllMocks();vi.stubEnv("WALLET_EXPORT_ENABLED","true");vi.stubEnv("WALLET_EXPORT_RUNTIME","shared");vi.stubEnv("WEB_AUTH_SECRET","private-server-secret");mocks.session.mockResolvedValue({xUserId:"1",provider:"x"});mocks.query.mockResolvedValue({eligible:true});});
...
it("derives the owner from the active session and ignores requested owner/address values",async()=>{
 ...
 expect(mocks.query).toHaveBeenCalledWith(expect.anything(),{secret:"private-server-secret",provider:"x",userId:"1"});
});
it("uses Telegram's numeric ID for a Telegram website session",async()=>{
 ...
 expect(mocks.query).toHaveBeenCalledWith(expect.anything(),expect.objectContaining({provider:"telegram",userId:"2"}));
});
it.each(["disabled","broker","signed-out","backend-failure"])("hides export for %s without leaking details",async mode=>{
 ...
 if(mode==="signed-out")mocks.session.mockRejectedValue(Error("private details"));if(mode==="backend-failure")mocks.query.mockRejectedValue(Error("private details"));
 ...
 if(mode!=="backend-failure")expect(mocks.query).not.toHaveBeenCalled();
});
