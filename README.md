...
The application has been rebranded as Argos Bot. Mainnet research and the implementation plan are in [docs/arc/IMPLEMENTATION.md](docs/arc/IMPLEMENTATION.md). Active Arc transactions use the chain-5042 trading and settlement modules. Retained legacy contracts and operator tools are not verified Arc deployments. See docs/readiness-followup-2026-09-11.md for the latest readiness review.
...
The `/otc` market supports Arc USDC listings priced in Base ETH, partial fills, dedicated CDP position escrow, and a 1.5% buyer fee after the premium. Buyers and sellers pay their gas. `/wallet` provides Buy, Sell, Swap, and Send controls with transaction history. Base ETH withdrawals are also supported through Telegram. OTC remains website-only. See [OTC setup and settlement](docs/otc/IMPLEMENTATION.md).
